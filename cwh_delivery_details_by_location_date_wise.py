...
def fetch_cwh_details_by_loc_date_unit_price(loc_id, from_date, to_date):
    df_list: list = []
    food_total: list = []
    total_amount: list = []
    cleaning_total: list = []
    ...
    try:
        with get_database_connection_e_eiis() as conn:
            ...
            if len(loc_id) != 0:
                sql_query = """ SELECT 
                            ...
                            WHERE  
                                cwhhead.ORD_LOC_ID = %s
                                AND cwhhead.DEL_DATE BETWEEN %s AND %s;

                            """
                cursor.execute(sql_query, (loc_id, from_date, to_date))
            else:
                sql_query = """ SELECT 
                                                cwhhead.CWH_DEL_ID, 
                                                cwhhead.ORD_LOC_ID, 
            ...
            if len(nested_list) == 0:
                print(
                    f'NO DATA AVAILABLE FOR CWH DELIVERY FOR THE SELECTED DATE FROM  -> {from_date}  TO {to_date} AND THE LOCATION ID --> {loc_id}')
                status = "failed"
                return (status, df_list, other_total, disposal_total, cleaning_total, food_total, total_amount,
                        nested_list, delivery_type)
        ...
        df_query = """
                    ...
                    ROUND((cwhdetail.QTY_UNIT * cwhdetail.IP_UNIT), 4) AS Total_Amount, 
...
def fetch_cwh_details_by_loc_date_old(loc_id, from_date, to_date):
    df_list: list = []
    food_total: list = []
    total_amount: list = []
    cleaning_total: list = []
    ...
    try:
        ...
        df_query = """
                    ...
                    cwhdetail.BATCH_NO, cwhdetail.QTY, cwhdetail.IP, 
                    ROUND((cwhdetail.QTY * cwhdetail.IP), 3) AS Total_Amount , mstitem.ACCOUNT_NAME 
...
def create_cwh_pdf_for_ind_loc_by_date(df_list, other_total, disposal_total, cleaning_total, food_total, total_amount,
                                       nested_list,
                                       delivery_type):
    pdf_file_name = []
    page = 0
...
def fetch_direct_delivery_details_for_ind_loc_date_wise(loc_id, from_date, to_date):
    df_list: list = []
    food_total: list = []
    total_amount: list = []
    cleaning_total: list = []
...
def fetch_direct_delivery_details_date_wise_old(loc_id, from_date, to_date):
    df_list: list = []
    food_total: list = []
    total_amount: list = []
    cleaning_total: list = []
