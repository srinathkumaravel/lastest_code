...
def fetch_cwh_details_by_ind_loc(month, year, loc_id):
    df_list: list = []
    food_total: list = []
    total_amount: list = []
    cleaning_total: list = []
        ...
        with get_database_connection_e_eiis() as conn:
            ...
            if len(loc_id) != 0:
                sql_query = """ SELECT cwhhead.CWH_DEL_ID, cwhhead.ORD_LOC_ID, loc.Location_Name, cwhhead.STORE_LOC_ID, 
                                ...
                                MONTH(cwhhead.PERIOD) = %s AND cwhhead.ORD_LOC_ID = %s;
                            """
                cursor.execute(sql_query, (year, month, loc_id))
            else:
                sql_query = """ SELECT cwhhead.CWH_DEL_ID, cwhhead.ORD_LOC_ID, loc.Location_Name, cwhhead.STORE_LOC_ID, 
                                                cwhhead.DEL_DATE FROM cwhdelhead AS cwhhead INNER JOIN location AS loc ON 
                                                loc.Location_ID = cwhhead.ORD_LOC_ID WHERE YEAR(cwhhead.PERIOD) = %s AND 
            ...
            if len(nested_list) == 0:
                print(
                    f'NO DATA AVAILABLE FOR CWH DELIVERY FOR THE SELECTED PERIOD MONTH -> {month} & YEAR -> {year} AND THE LOCATION ID --> {loc_id}')
                status = "success"
                message = "No data available"
                return (status, df_list, other_total, disposal_total, cleaning_total, food_total, total_amount,
                        nested_list, formatted_date, delivery_type, message)
...
def fetch_cwh_details_old(month, year, loc_id):
    df_list: list = []
    food_total: list = []
    total_amount: list = []
    cleaning_total: list = []
...
def draw_header_and_details(c, width, height, formatted_date, person_name, rec_center_no, cwh_delivery_no,
    ...
    store_delivery_text = "Invoice ID :"
...
def create_cwh_pdf_for_ind_loc(df_list, other_total, disposal_total, cleaning_total, food_total, total_amount,
                               nested_list,
        ...
        for df, other_tot, dis_tot, cleaning_tot, food_tot, tot_amount, nest_list in zip(df_list, other_total,
            ...
            for i, chunk in enumerate(chunks):
                ...
                if i < len(chunks) - 1:
                    ...
                    total_values = [food_tot, cleaning_tot, dis_tot, other_tot, tot_amount]
                    # Font settings for normal text (values)
                    c.setFont("Helvetica-Bold", 8)
...
def fetch_direct_delivery_details_for_ind_loc(month, year, loc_id):
    df_list: list = []
    food_total: list = []
    total_amount: list = []
    cleaning_total: list = []
    ...
    try:
        ...
        if len(nested_list) == 0:
            status = "success"
            message = "No data available"
            print(f'NO DATA AVAILABLE FOR DIRECT DELIVERY FOR THE SELECTED PERIOD MONTH -> {month} & YEAR -> {year} & LOCATION ID -> {loc_id}')
            return (status, df_list, other_total, disposal_total, cleaning_total, food_total, total_amount,
                    nested_list, formatted_date, delivery_type, message)
        ...
        df_query = """
                     SELECT 
                            ...
                            detail.EXPIRY_DATE, 
                            detail.BATCH_NO, 
                            ROUND(detail.QTY_UNIT, 5) AS QTY_UNIT, 
                            CASE
                                WHEN l.Payment_Type = 1 THEN ROUND(SUM(GP_UNIT), 3)
                                ELSE ROUND(item.IP02 / mitem.PACKAGE_BASE_FACTOR, 3)
                            END AS GP_UNIT, 
                            CASE
                                WHEN l.Payment_Type = 1 THEN  ROUND(SUM(GP_UNIT*QTY_UNIT), 3)
                                ELSE ROUND((detail.QTY_UNIT * (item.IP02 / mitem.PACKAGE_BASE_FACTOR)), 3) 
                            END AS Total_Amount,
                            CASE
                                WHEN it_acc.ACCOUNT_NAME IN ('FOOD', 'CLEANING', 'DISPOSABLES') THEN it_acc.ACCOUNT_NAME
                                ELSE 'OTHERS'
                        ...
                        INNER JOIN 
                            suppdelhead AS sdh ON sdh.GRN_ID = detail.GRN_ID    
                        INNER JOIN 
                            location AS l ON l.Location_ID = sdh.ORD_LOC_ID     
                        INNER JOIN 
                            item AS item ON item.Item_ID = detail.ITEM_ID 
                        INNER JOIN 
                            mst_item_master AS mitem ON mitem.ITEM_CODE = detail.ITEM_ID     
                        INNER JOIN 
...
def fetch_direct_delivery_details_old(month, year, loc_id):
    df_list: list = []
    food_total: list = []
    total_amount: list = []
    cleaning_total: list = []
    ...
    try:
        ...
        df_query = """
                    SELECT 
                            ...
                            detail.QTY, 
                            CASE
                              WHEN l.Payment_Type = 1 THEN ROUND(detail.GP, 3)
                              ELSE item.IP02
                            END AS GP,
                            CASE
                              WHEN l.Payment_Type = 1 THEN  ROUND(detail.GP * detail.QTY, 3)
                              ELSE ROUND((detail.QTY * item.IP02), 3) 
                            END AS Total_Amount, 
                            CASE
                                WHEN it_acc.ACCOUNT_NAME IN ('FOOD', 'CLEANING', 'DISPOSABLES') THEN it_acc.ACCOUNT_NAME
                                ELSE 'OTHERS'
