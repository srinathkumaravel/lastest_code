...
def fetch_supplier_invoice_data(month, year):
    unique_supplier_ids = None
    unique_supplier_name = None
    unique_supplier_inv_id = None
    dfs = None
    status = None
    try:
        engine = get_database_engine_e_eiis()
        df_query = """SELECT DISTINCT head.SUPPLIER_ID, sup.Supplier_Name, head.SUPP_INV_ID, head.SUPP_INV_DATE, head.ORD_LOC_ID, loc.Location_Name,head.GRN_ID, det.ITEM_ID, it.Item_Name, det.PACKAGE_ID, det.QTY, det.GP, det.ACTUAL_INV FROM 
        suppdelhead AS head INNER JOIN suppliers AS sup ON sup.Supplier_ID = head.SUPPLIER_ID INNER JOIN suppdeldetail AS det ON det.GRN_ID = head.GRN_ID INNER JOIN item AS it ON it.Item_ID = det.ITEM_ID
        INNER JOIN location AS loc ON loc.Location_ID = head.ORD_LOC_ID WHERE MONTH(PERIOD) = %s AND YEAR(PERIOD) = %s ORDER BY head.SUPPLIER_ID, head.SUPP_INV_ID, head.ORD_LOC_ID, head.GRN_ID, det.ITEM_ID;"""
        ...
        unique_supplier_name = df['Supplier_Name'].unique()
        unique_supplier_inv_id = df['SUPP_INV_ID'].unique()
        # unique_supplier_date_id = df['SUPP_INV_DATE'].unique()
        print(unique_supplier_ids)
        print(unique_supplier_name)
        print(unique_supplier_inv_id)
        # Sort the DataFrame by 'Supplier name'
        df_sorted = df.sort_values(by='SUPPLIER_ID')

        # Group by 'SUPP_INV_ID' and 'ORD_LOC_ID'
        ...
        for (supp_id, location_id), group in grouped:
        ...
        for sup_id, sup_name in zip(unique_supplier_ids, unique_supplier_name):
            ...
            print('Supplier Name - ', sup_name)
            for inv_no in unique_supplier_inv_id:
                print('Supp invoice num : -', inv_no)
                for sup_df in dfs:
                    # print('Supp invoice num : -', inv_no)
                    ...
                    if inv_no == first_element_supp_inv_id and sup_id == first_elemnt_sup_id:
                        ...
                        sup_df.rename(columns=rename_dict, inplace=True)
                        print(sup_df)
                        print(
                            "*************************************************************************************************")
                        status = "success"
        ...
        status = "failed"
    return unique_supplier_ids, unique_supplier_name, unique_supplier_inv_id, dfs, status


def fetch_supplier_invoice_data_by_supplier_id(month, year, supp_id):
    unique_supplier_ids = None
    unique_supplier_name = None
    unique_supplier_inv_id = None
    dfs = None
    ...
    try:
        ...
        df_query = """SELECT DISTINCT 
                    ...
                    """
        # Execute the query and retrieve the data as a DataFrame
        df = pd.read_sql_query(df_query, engine, params=(month, year, supp_id))
        # Assuming you already have a DataFrame df
        unique_supplier_ids = df['SUPPLIER_ID'].unique()
        unique_supplier_name = df['Supplier_Name'].unique()
        unique_supplier_inv_id = df['SUPP_INV_ID'].unique()
...
def fetch_supplier_invoice_data_by_date_and_supplier_id(from_date, to_date, supp_id):
    unique_supplier_ids = None
    unique_supplier_name = None
    unique_supplier_inv_id = None
    dfs = None
    ...
    try:
        ...
        df_query = """SELECT DISTINCT 
                    ...
                    """
        # Execute the query and retrieve the data as a DataFrame
        df = pd.read_sql_query(df_query, engine, params=(from_date, to_date, supp_id))
        # Assuming you already have a DataFrame df
        unique_supplier_ids = df['SUPPLIER_ID'].unique()
        unique_supplier_name = df['Supplier_Name'].unique()
        unique_supplier_inv_id = df['SUPP_INV_ID'].unique()
