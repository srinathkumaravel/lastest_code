...
def fetch_cwh_details(month, year):
    df_list: list = []
    food_total: list = []
    total_amount: list = []
    cleaning_total: list = []
...
def create_cwh_pdf(df_list, other_total, disposal_total, cleaning_total, food_total, total_amount, nested_list,
    ...
    try:
        for df, other_tot, dis_tot, cleaning_tot, food_tot, tot_amount, nest_list in zip(df_list, other_total,
            ...
            for i, chunk in enumerate(chunks):
                ...
                # Create the table
                colWidths = [1.4 * cm, 3.2 * cm, 2.7 * cm, 1.4 * cm, 1.2 * cm, 1.4 * cm, 1.4 * cm, 1.5 * cm, 1.5 * cm,
                             1.5 * cm, 1.5 * cm, 1.6 * cm]
                ...
                if i < len(chunks) - 1:
                    ...
                    label_x_values = [left_margin + 13.1 * cm, left_margin + 14.8 * cm, left_margin + 16.3 * cm,
                                      left_margin + 17.8 * cm, left_margin + 19.2 * cm]
                    ...
                    c.line(left_margin + 12.8 * cm, line_y, left_margin + 20.2 * cm, line_y)  # Draw the line
...
def fetch_direct_delivery_details(month, year):
    df_list: list = []
    food_total: list = []
    total_amount: list = []
    cleaning_total: list = []
