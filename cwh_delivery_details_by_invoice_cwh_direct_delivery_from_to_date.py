...
def fetch_cwh_details_from_to_date(from_date, to_date, month, year):
    df_list: list = []
    food_total: list = []
    total_amount: list = []
    cleaning_total: list = []
    ...
    try:
        with get_database_connection_e_eiis() as conn:
            ...
            if len(month) == 0 and len(year) == 0:
                sql_query = """ SELECT 
                                    ...
                                    cwhhead.ORD_LOC_ID, 
                                    loc.Location_Name, 
                                    cwhhead.STORE_LOC_ID, 
                                    cwhhead.DEL_DATE 
                                    FROM 
                                    cwhdelhead AS cwhhead 
                                    INNER JOIN 
                                    location AS loc ON loc.Location_ID = cwhhead.ORD_LOC_ID 
                                    WHERE  
                                    cwhhead.DEL_DATE BETWEEN %s AND %s;
                            """
                ...
                sql_query = """ SELECT 
                                ...
                                INNER JOIN 
                                    location AS loc 
                                    ON loc.Location_ID = cwhhead.ORD_LOC_ID 
                                WHERE  
                                    MONTH(cwhhead.PERIOD) = %s 
                                    AND YEAR(cwhhead.PERIOD) = %s;
                            """
            ...
            if len(nested_list) == 0:
                print(
                    f'NO DATA AVAILABLE FOR CWH DELIVERY FOR THE SELECTED FROM DATE -> {from_date} & TO Date -> {to_date}')
                status = "success"
                message = "No data available"
                return (status, df_list, other_total, disposal_total, cleaning_total, food_total, total_amount,
                        nested_list, delivery_type, message)
        engine = get_database_engine_e_eiis()
        df_query = """
                    SELECT 
                        cwhdetail.ITEM_ID, 
                        item.Item_Name, 
                        cwhdetail.PACKAGE_ID, 
                        cwhdetail.EXPIRY_DATE, 
                        cwhdetail.BATCH_NO, 
                        cwhdetail.QTY, 
                        cwhdetail.IP, 
                        ROUND((cwhdetail.QTY * cwhdetail.IP), 3) AS Total_Amount, 
                        CASE
                            WHEN it_acc.ACCOUNT_NAME IN ('FOOD', 'CLEANING', 'DISPOSABLES') THEN it_acc.ACCOUNT_NAME
                            ELSE 'OTHERS'
                        END AS ACCOUNT_NAME
                    FROM 
                        cwhdeldetail AS cwhdetail 
                    INNER JOIN 
                        item AS item ON item.Item_ID = cwhdetail.ITEM_ID 
                    INNER JOIN 
                        mst_item_category AS m ON LEFT(cwhdetail.ITEM_ID, 2) = m.ITEM_CAT_PK 
                    INNER JOIN 
        ...
        for sublist in nested_list:
            ...
            # Convert 'col1' to numeric and fill NaN with 0
            concatenated_df[['QTY']] = concatenated_df[['QTY']].infer_objects().fillna(0)
            concatenated_df[['IP']] = concatenated_df[['IP']].infer_objects().fillna(0)
            concatenated_df[['FOOD']] = concatenated_df[['FOOD']].infer_objects().fillna(0)
            concatenated_df[['CLEANING']] = concatenated_df[['CLEANING']].infer_objects().fillna(0)
            concatenated_df[['DISPOSABLES']] = concatenated_df[['DISPOSABLES']].infer_objects().fillna(0)
            concatenated_df[['Total_Amount']] = concatenated_df[['Total_Amount']].infer_objects().fillna(0)
            concatenated_df[['OTHERS']] = concatenated_df[['OTHERS']].infer_objects().fillna(0)
            ...
            # Fill NaN values with 0.000 for the specified columns
            columns_to_fill = ['Issue Qty', 'Unit Price', 'Food', 'Cleaning', 'Disposal', 'Others', 'Total Amount']
            concatenated_df[columns_to_fill] = concatenated_df[columns_to_fill].fillna(0.000)

            # Convert columns to numeric to handle potential dtype issues
            concatenated_df[columns_to_fill] = concatenated_df[columns_to_fill].apply(pd.to_numeric)

            # Ensure columns from 5th column onward are numeric (float)
            concatenated_df.iloc[:, 4:] = concatenated_df.iloc[:, 4:].apply(pd.to_numeric, errors='coerce')

            # Specify columns to format
            columns_to_format = ['Issue Qty', 'Unit Price', 'Food', 'Cleaning', 'Disposal', 'Others', 'Total Amount']

            # Apply formatting to specified columns
            concatenated_df[columns_to_format] = concatenated_df[columns_to_format].apply(
                lambda x: x.map('{:.3f}'.format))
            # Set 'Batch No' column to empty strings
            concatenated_df['Batch No'] = ''
...
def fetch_cwh_details_unit_from_to_date(from_date, to_date, month, year):
    df_list: list = []
    food_total: list = []
    total_amount: list = []
    cleaning_total: list = []
    ...
    try:
        ...
        df_query = """
                    SELECT 
                        ...
                        cwhdetail.EXPIRY_DATE, 
                        cwhdetail.BATCH_NO, 
                        ROUND(cwhdetail.QTY_UNIT, 3) AS QTY_UNIT, 
                        ROUND(cwhdetail.IP_UNIT, 3) AS IP_UNIT,
                        ROUND((cwhdetail.QTY_UNIT * cwhdetail.IP_UNIT), 3) AS Total_Amount, 
                        CASE
                            WHEN it_acc.ACCOUNT_NAME IN ('FOOD', 'CLEANING', 'DISPOSABLES') THEN it_acc.ACCOUNT_NAME
                            ELSE 'OTHERS'
                        END AS ACCOUNT_NAME
        ...
        for sublist in nested_list:
            ...
            concatenated_df = pd.concat([df, df_1], axis=1)
            # Convert 'col1' to numeric and fill NaN with 0
            concatenated_df[['QTY_UNIT']] = concatenated_df[['QTY_UNIT']].infer_objects().fillna(0)
            concatenated_df[['IP_UNIT']] = concatenated_df[['IP_UNIT']].infer_objects().fillna(0)
            concatenated_df[['FOOD']] = concatenated_df[['FOOD']].infer_objects().fillna(0)
            concatenated_df[['CLEANING']] = concatenated_df[['CLEANING']].infer_objects().fillna(0)
            concatenated_df[['DISPOSABLES']] = concatenated_df[['DISPOSABLES']].infer_objects().fillna(0)
            concatenated_df[['Total_Amount']] = concatenated_df[['Total_Amount']].infer_objects().fillna(0)
...
def merge_pdfs_date_wise(pdf_files, output_path):
    try:
...
def draw_header_and_details(c, width, height, person_name, rec_center_no, cwh_delivery_no,
    ...
    third_elemnt = "CWH Delivery Note/Invoice"
...
def create_cwh_dd_pdf_date_wise(df_list, other_total, disposal_total, cleaning_total, food_total, total_amount,
                                nested_list,
                                delivery_type):
    pdf_file_name = []
    page = 0
    ...
    try:
        for df, other_tot, dis_tot, cleaning_tot, food_tot, tot_amount, nest_list in zip(df_list, other_total,
            ...
            directory_path = r"C:\Users\Administrator\Downloads\eiis\CWH"
            ...
            for i, chunk in enumerate(chunks):
                ...
                # Create the table
                colWidths = [1.58 * cm, 6.8 * cm, 3.0 * cm, 1.6 * cm, 1.2 * cm, 1.79 * cm]
                ...
                df_table.setStyle([
                    ...
                    ('FONTSIZE', (0, 1), (-1, -1), 6),  # Font size for body cells
                    ('ALIGN', (1, 0), (1, -1), 'LEFT'),  # Left align text in the 2nd column
                    ('ALIGN', (5, 0), (10, -1), 'RIGHT'),  # Right align text from column 6 to 11
                ])
                ...
                if i < len(chunks) - 1:
                    ...
                    label_x_values = [left_margin + 17.9 * cm, left_margin + 19.8 * cm, left_margin + 21.6 * cm,
                                      left_margin + 23.5 * cm, left_margin + 25.1 * cm]
                    ...
                    c.line(left_margin + 12.8 * cm, line_y, left_margin + 26.5 * cm, line_y)  # Draw the line
        ...
        # Merge the PDFs
        status, output_path = merge_pdfs_date_wise(pdf_file_name, output_path)
...
def fetch_direct_delivery_details_from_to_date(from_date, to_date, month, year):
    df_list: list = []
    food_total: list = []
    total_amount: list = []
    cleaning_total: list = []
    ...
    try:
        with get_database_connection_e_eiis() as conn:
            ...
            if len(month) == 0 and len(year) == 0:
                sql_query = """ SELECT 
                                    ...
                                    head.SUPP_DEL_DATE,
                                    head.SUPP_INV_ID AS STORE_LOC_ID
                                FROM 
                                    suppdelhead AS head
                                INNER JOIN 
                                    location AS loc 
                                ...
                                LEFT JOIN 
                                    entityeiis AS eii 
                                    ON eii.CWH = head.ORD_LOC_ID
                                WHERE 
                                    eii.CWH IS NULL 
                                    AND head.GRN_DATE BETWEEN %s AND %s;
                            """
                cursor.execute(sql_query, (from_date, to_date))
            else:
                sql_query = """ SELECT 
        ...
        if len(nested_list) == 0:
            ...
            message = "No data available"
            print(
                f'NO DATA AVAILABLE FOR DIRECT DELIVERY FOR THE SELECTED PERIOD FROM DATE -> {from_date} AND TO DATE -> {to_date}')
            return (status, df_list, other_total, disposal_total, cleaning_total, food_total, total_amount,
                    nested_list, delivery_type, message)
        ...
        df_query = """
                    SELECT 
                        ...
                        detail.QTY, 
                        detail.GP, 
                        ROUND((detail.QTY * detail.GP), 3) AS Total_Amount, 
                        CASE
                            WHEN it_acc.ACCOUNT_NAME IN ('FOOD', 'CLEANING', 'DISPOSABLES') THEN it_acc.ACCOUNT_NAME
                            ELSE 'OTHERS'
                        END AS ACCOUNT_NAME
                    ...
                    INNER JOIN 
                        mst_item_category AS m ON LEFT(detail.ITEM_ID, 2) = m.ITEM_CAT_PK 
                    INNER JOIN 
                        mst_item_account AS it_acc ON it_acc.ITEM_ACCOUNT_PK = m.ACCOUNT_FK 
                    WHERE 
                        detail.GRN_ID = %s;
        ...
        for sublist in nested_list:
            ...
            # Convert 'col1' to numeric and fill NaN with 0
            concatenated_df[['QTY']] = concatenated_df[['QTY']].infer_objects().fillna(0)
            concatenated_df[['GP']] = concatenated_df[['GP']].infer_objects().fillna(0)
            concatenated_df[['FOOD']] = concatenated_df[['FOOD']].infer_objects().fillna(0)
            concatenated_df[['CLEANING']] = concatenated_df[['CLEANING']].infer_objects().fillna(0)
            concatenated_df[['DISPOSABLES']] = concatenated_df[['DISPOSABLES']].infer_objects().fillna(0)
            concatenated_df[['Total_Amount']] = concatenated_df[['Total_Amount']].infer_objects().fillna(0)
...
def fetch_direct_delivery_details_unit_from_to_date(from_date, to_date, month, year):
    df_list: list = []
    food_total: list = []
    total_amount: list = []
    cleaning_total: list = []
    ...
    try:
        ...
        for sublist in nested_list:
            ...
            # Convert 'col1' to numeric and fill NaN with 0
            concatenated_df[['QTY_UNIT']] = concatenated_df[['QTY_UNIT']].infer_objects().fillna(0)
            concatenated_df[['GP_UNIT']] = concatenated_df[['GP_UNIT']].infer_objects().fillna(0)
            concatenated_df[['FOOD']] = concatenated_df[['FOOD']].infer_objects().fillna(0)
            concatenated_df[['CLEANING']] = concatenated_df[['CLEANING']].infer_objects().fillna(0)
            concatenated_df[['DISPOSABLES']] = concatenated_df[['DISPOSABLES']].infer_objects().fillna(0)
            concatenated_df[['Total_Amount']] = concatenated_df[['Total_Amount']].infer_objects().fillna(0)
